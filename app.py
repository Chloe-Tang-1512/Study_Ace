...
def edit_set(set_id):
    ...
    if request.method == 'POST':
        ...
        for term, definition in zip(terms, definitions):
            if term and definition:
                card = Flashcard(term=term, definition=definition, set_id=flashcard_set.id)
                db.session.add(card)
        db.session.commit()
...
def flashcard_practise(set_id):
    ...
    if mode == 'classic':
        ...
        if len(cards) < 4:
            flash('Need at least 4 cards for multiple choice mode.')
            return redirect(url_for('dashboard'))
        ...
        if request.method == 'POST':
            ...
            if user_input.lower() == correct_word.lower():
                ...
                flash(f'Incorrect. The correct word was: {correct_word}')
            idx += 1
...
def set_theme(theme):
    if theme not in ['blue', 'red', 'green']:
        flash('Invalid theme.')
